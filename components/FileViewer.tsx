...
const FileViewer: React.FC<FileViewerProps> = ({ file, onSave, onDirtyChange }) => {
  ...
  // Track reported dirty state to avoid spamming parent
  const reportedDirtyRef = React.useRef<boolean>(false);

  // Check dirty state
  useEffect(() => {
    const isDirty = content !== (file.content || '');
    // Only report if changed
    if (reportedDirtyRef.current !== isDirty) {
      onDirtyChange(file.id, isDirty);
      reportedDirtyRef.current = isDirty;
    }

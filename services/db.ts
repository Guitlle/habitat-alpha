import { WorkData, Project, Epic, Task, FileNode, CalendarEvent } from '../types';
import { initialWorkData, initialFiles, initialEvents } from './mockDataService';

const DB_NAME = 'CognitiveWorkspaceDB';
const DB_VERSION = 3; // Bumped version for 'events' store
...
export const db = {
  ...
  init: async (): Promise<{ workData: WorkData, files: FileNode[], events: CalendarEvent[] }> => {
    const database = await db.open();
    const tx = database.transaction(['projects', 'epics', 'tasks', 'files', 'events'], 'readwrite');
  ...
  getAllData: async (existingDb?: IDBDatabase): Promise<{ workData: WorkData, files: FileNode[], events: CalendarEvent[] }> => {
     const database = existingDb || await db.open();
     const tx = database.transaction(['projects', 'epics', 'tasks', 'files', 'events'], 'readonly');
    ...
    const events = await new Promise<CalendarEvent[]>((resolve, reject) => {
     ...
     return { workData: { projects, epics, tasks }, files, events };
  },
